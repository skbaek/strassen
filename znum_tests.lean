...
def const_mat (c : znum) : ∀ k, matrix znum (2^k) (2^k)
| 0 := vector.singleton (vector.singleton c)
| (k+1) := quadruple (const_mat k, const_mat k, const_mat k, const_mat k)

def arith_prog : ∀ k m : nat, vector znum m
| k 0     := nil 
| k (m+1) := cons k (arith_prog (k+1) m)

def arith_prog_mat_core : ∀ k m n : nat, matrix znum m n
| k 0     n := nil
| k (m+1) n := cons (arith_prog k n) (arith_prog_mat_core (k+1) m n) 

def arith_prog_mat (m : nat) : matrix znum (2^m) (2^m) := 
arith_prog_mat_core 0 (2^m) (2^m) 

def fib_vec : ∀ k : nat, ∀ i j : znum, vector znum k 
| 0 _ _     := nil
| (k+1) i j := (i+j)::(fib_vec k j (i+j))

def fib_mat_core : ∀ m n : nat, ∀ i j : znum, matrix znum m (n+2)
| 0     _ _ _ := nil
| (m+1) n i j := (i::j::fib_vec n i j)::(fib_mat_core m n j (i+j))

def fib_mat (m : nat) : matrix znum (2^m) (2^m) := 
match (2^m) with 
| 0 := nil 
...
def test_mat := matrix.map (λ x, (100000 : znum) * x) (arith_prog_mat size)
