...
meta def debug [has_repr α] : ∀ {k : nat}, 
  matrix α (2^k) (2^k) → matrix α (2^k) (2^k) → tactic (matrix α (2^k) (2^k)) 
| 0 A B := return $ vector.singleton (vector.singleton (A.head.head * B.head.head))
| (k+1) A B :=
  let (A₁₁, A₁₂, A₂₁, A₂₂) := quadrisect A in
  let (B₁₁, B₁₂, B₂₁, B₂₂) := quadrisect B in
  let S₁ := A₂₁ + A₂₂ in
